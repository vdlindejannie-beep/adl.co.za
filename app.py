...
PRODUCTS = [
    ...
    {"id": 38, "name": "Leather Belt", "category": "Belts", "price": 200.00, "images": ["Screenshot 2026-08-14 140410.png"], "description": "Handmade full-grain leather belt.", "colors": ["Black", "Brown"], "sizes": ["Small (30-32\")", "Medium (34-36\")", "Large (38-40\")", "X-Large (42-44\")"]},
            ...
            if response.data:
                for order in response.data:
                    if order.get('timestamp'):
                        try:
                            order['timestamp'] = datetime.fromisoformat(order['timestamp'].replace('Z', '+00:00'))
                        except Exception:
                            pass
                return response.data
        except Exception as e:
...
def update_order_status_in_db(order_id, status):
    if supabase:
        try:
            supabase.table("orders").update({"status": status, "shipped_at": datetime.now().isoformat()}).eq("id", order_id).execute()
        except Exception as e:
...
def send_email_https(subject, recipient_email, body_text):
    ...
    payload = {"sender": {"name": "Leather by Annuschka", "email": STUDIO_EMAIL}, "to": [{"email": recipient_email}], "subject": subject, "textContent": body_text}
...
def generate_summary_email(period_name):
    with app.app_context():
        ...
        if period_name == "Weekly":
            ...
            recent_orders = [o for o in orders if isinstance(o.get('timestamp'), datetime) and o.get('timestamp') >= cutoff_date]
        else:
            first_of_this_month = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            last_day_of_prev_month = first_of_this_month - timedelta(days=1)
            recent_orders = [o for o in orders if isinstance(o.get('timestamp'), datetime) and o.get('timestamp').month == last_day_of_prev_month.month]
...
def add_to_cart(product_id):
    ...
    if product:
        ...
        if existing:
            ...
            cart.append({'id': product['id'], 'name': product['name'], 'price': unit_price, 'image': product['images'][0], 'color': color, 'size': size, 'available_colors': product.get('colors', []), 'available_sizes': product.get('sizes', []), 'quantity': quantity})
        session['cart'] = cart
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        return render_template('payfast_redirect.html', payfast_url=PAYFAST_URL, merchant_id=PAYFAST_MERCHANT_ID, merchant_key=PAYFAST_MERCHANT_KEY, return_url=url_for('payment_success', _external=True), cancel_url=url_for('cart', _external=True), notify_url=url_for('payfast_itn', _external=True), m_payment_id=order_id, amount=f"{total:.2f}", item_name=f"Order #{order_id}", name_first=first_name, name_last=last_name, email_address=customer_email, cell_number=request.form.get('phone', ''))

    return render_template('checkout.html', cart_items=cart_items, subtotal=subtotal, shipping=shipping_info['price'], total=total, paxi_options=PEPAXI_OPTIONS)
